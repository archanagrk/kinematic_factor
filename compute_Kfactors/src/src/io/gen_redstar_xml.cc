...
int main(int argc, char** argv){
  ...
  for(int k =1;k <= pts;k++){
    for(int i = 1; i <= npt; i++ ){
      ...
      read(xml_kf_in, "/kfac/elem["+std::to_string(k)+"]/elem["+std::to_string(i)+"]/msq", msq_tmp);
      ...
      if(msq_tmp != 0){op_tmp.ops[1]= KeyParticleOp_t("Kneg_proj0_p" + std::to_string(canon_mom_tmp[0]) + std::to_string(canon_mom_tmp[1]) + std::to_string(canon_mom_tmp[2]) + "_H"+ std::to_string(lam_tmp) + irrep_tmp, "", canon_mom_tmp);}
