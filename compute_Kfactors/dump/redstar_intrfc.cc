                                  ...
                                  for(int row_curr = 1; row_curr <= irrepRows(*r); row_curr++){
                                    for(int row3 = 1; row3 <= irrepRows(*q); row3++){
                                      ...
                                      XMLArray::Array<int> canon_mom2 = Hadron::canonicalOrder(ToArray::toArray(mom_curr));
                                        ...
                                        Coeff = KFac::KFacSVV(qp,qm,Sub1,SubCurr,Sub3);
                                        if(std::real(Coeff) || std::imag(Coeff)){
                                            if(canon_mom2[0] == mom_curr(0) && canon_mom2[1]== mom_curr(1) && canon_mom2[2] == mom_curr(2)){
                                                ...
                                                cout << "The factor is:" << Coeff << "\n";
					                                ...
					                                key.npoint[1].irrep.flavor.twoI = 2;
					                                key.npoint[1].irrep.flavor.threeY = 0;
					                                key.npoint[1].irrep.flavor.twoI_z = 2;
                                                    ...
                                                    if(mom1_sq != 0){
                                                        op1.ops[1]= KeyParticleOp_t("pion_proj0_p" + std::to_string(canon_mom1[0]) + std::to_string(canon_mom1[1]) + std::to_string(canon_mom1[2]) + "_H0" + LG1 + rep1.irrep, "", canon_mom1);}
                                                    else{op1.ops[1]= KeyParticleOp_t("pion_proj0_p" + std::to_string(canon_mom1[0]) + std::to_string(canon_mom1[1]) + std::to_string(canon_mom1[2]) + "_"+rep1.irrep, "", canon_mom1);}
                                                    ...
                                                    if(mom_curr_sq != 0){
                                                        op2.ops[1] = KeyParticleOp_t("omegal_rhoxD0_J0__J1_H"+ std::to_string(helicity_curr) + LG_curr + rep_curr.irrep , "", canon_mom2);}
                                                    ...
                                                    key.npoint[3].irrep.flavor.threeY = 0;
                                                    key.npoint[3].irrep.flavor.twoI_z = 2;
                                                    ...
                                                    if(mom3_sq != 0){
                                                        ...
                                                        op3.ops[1] = KeyParticleOp_t("rho_proj0_p"+ std::to_string(canon_mom3[0]) + std::to_string(canon_mom3[1]) + std::to_string(canon_mom3[2]) + "_H" + std::to_string(helicity3) + LG3+ rep3.irrep, "", canon_mom3);
                                                        //op3.ops[1] = KeyParticleOp_t("Kneg_rhoxD0_J0__J1_H"+ std::to_string(helicity3) + LG3+ rep3.irrep, "", canon_mom3);
                                                    }
                                                    else{
                                                      
                                                      
                                                      op3.ops[1] = KeyParticleOp_t("rho_proj0_p"+ std::to_string(canon_mom3[0]) + std::to_string(canon_mom3[1]) + std::to_string(canon_mom3[2]) + "_" + rep3.irrep, "", canon_mom3);
                                                      
                                                      //op3.ops[1] = KeyParticleOp_t("Kneg_rhoxD0_J0__J1_" + rep3.irrep, "", canon_mom3);
                                                    }
