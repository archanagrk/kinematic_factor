                 ...
                 for(int n=-int(sqrt(max_mom3)); n<=int(sqrt(max_mom3)); n++){
                  ...
                  if(max_mom3 >= mom3_sq){
                   ...
                   if(max_mom2 >= mom_curr_sq){
                    ...
                    VectorXd  n_qp(4,1);  VectorXd  n_qm(4,1);  // q+ = (p1-p2) q- = (p1+p2)
                       
                    Ph::phChars phase;
                    
                    phase = Ph::phaseFactor(two_J1, two_J3, two_J2, mom1, mom3, 1);
                                ...
                                for(int row1 = 1; row1 <= irrepRows(*p); row1++){     // Looping over all irrep rows at source, sink & insertion
                                  for(int row_curr = 1; row_curr <= irrepRows(*r); row_curr++){
                                    for(int row3 = 1; row3 <= irrepRows(*q); row3++){
                                        ...
                                        map< int, Eigen::MatrixXcd >Sub1 = Subduce_with_pol(n_mom1_sq, m1_sq, two_J1 , rep1, LG1, r1[0], r1[1], r1[2]);
                                        map< int, Eigen::MatrixXcd >Sub3 = Subduce_with_pol(n_mom3_sq, m3_sq, two_J3 , rep3, LG3, r3[0], r3[1], r3[2]);
                                        map< int, Eigen::MatrixXcd >SubCurr = Subduce_with_pol(n_mom_curr_sq, n_m_curr_sq, two_J2 , rep_curr, LG_curr, r_curr[0], r_curr[1], r_curr[2]);



                                        // if((mom1 == phase.mom1 && mom3 == phase.mom2 && mom_curr == n_mom_curr)) {
                                        //   Coeff = KFacSVV(qp,qm,Sub1,SubCurr,Sub3);
                                        // }
                                        // else if(mom_curr_sq){
                                          Coeff = KFac::KinematicFactorwithPhase(n_qp,n_qm,Sub1,SubCurr,Sub3,phase);
                                          // }
                                        // else{Coeff = KFacSVV(qp,qm,Sub1,SubCurr,Sub3);}
                                        // //else{Coeff = KinematicFactorwithPhase(qp,qm,Sub1,SubCurr,Sub3,phase);}
