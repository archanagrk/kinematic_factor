...
Ph::phChars Ph::phaseFactor(int twoJ1, int twoJ2, int twoJCurr, Eigen::Vector3d mom1, Eigen::Vector3d mom2, bool comp){
  ...
  Eigen::VectorXd n_mom1_f = R*mom1_f;
   ...
   Eigen::MatrixXd R_n_mom_curr = Rot::eulerRotMat(r_n_mom_curr[0],r_n_mom_curr[1],r_n_mom_curr[2]);
    ...
    cout << R_ref_mom2 << "\n";
     ...
     ans = PolVec::getPol4(sqw, app , ma , r_n_mom_curr[0],r_n_mom_curr[1],r_n_mom_curr[2]);
     cout << ans << "\n";
...
int main(){
  ...
  Ph::phChars chk;
  chk = Ph::phaseFactor(twoJ1,twoJ2,twoJCurr,mom1,mom2, 1);
