    ...
    switch(n)
    {
        ...
        case 2:
            for(int i = -twoJ; i <= twoJ; i = i+2){
                ...
                //cout << Hadron::Wigner_D(twoJ, twolam1, i, a1, b1, c1) << conj(Hadron::Wigner_D(twoJ, twolam2, i, a2, b2, c2)) << "\n";
            }
            break;
    ...
    for(int i = -twoJ2; i <= twoJ2; i = i+2){
      ...
      for(int i = -twoJCurr; i <= twoJCurr; i = i+2){
          for(int j = -twoJCurr; j <= twoJCurr; j = j+2){
            ...
            //cout << Hadron::Wigner_D(twoJCurr,  j, two_abs_lamCurr, r_mom_curr[0], r_mom_curr[1], r_mom_curr[2]) << "chw1" << Ph::comp_Wigner_d(twoJCurr, i, j, r2[0], r2[1], r2[2], r_mom2[0], r_mom2[1], r_mom2[2], 2) << "Dcurr" << conj(Hadron::Wigner_D(twoJCurr, i, two_abs_lamCurr, r_n_mom_curr[0], r_n_mom_curr[1], r_n_mom1[2])) << "chw" <<"\n";
      ...
      //complex<double> ph =  conj(D1)*D2;
      complex<double> ph =  conj(D1)*D2*conj(DCurr);
      //cout << two_abs_lamCurr << twoJCurr << "\n"; 
      //cout << ph << "\n";
      //complex<double> ph = 1;
